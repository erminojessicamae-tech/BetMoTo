...
if ($_SESSION['color_drought'] >= 3 && count($selected_colors) > 0) {
    ...
    $num1 = $color_map[$pity_color] ?? rand(1, 6); 
}
...
foreach ($selected_colors as $color) {
    ...
    // CRITICAL FIX: Standard Perya Multipliers
    if ($matches == 1) $total_won += ($bet_per_color * 2.0); // Returns Bet + 1x Profit
    elseif ($matches == 2) $total_won += ($bet_per_color * 3.0); // Returns Bet + 2x Profit
    elseif ($matches == 3) $total_won += ($bet_per_color * 4.0); // Returns Bet + 3x Profit
}

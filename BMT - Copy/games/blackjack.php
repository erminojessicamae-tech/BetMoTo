...
// Initialize streak tracker if it doesn't exist
if (!isset($_SESSION['bj_lose_streak'])) {
    $_SESSION['bj_lose_streak'] = 0;
}
...
if ($action === "hit") {
    ...
    $state["player_hand"][] = array_pop($state["deck"]); // Give card
    ...
    if ($pScore > 21) {
        $state["status"] = "lose";
        $message = "BUSTED!";
        $_SESSION['bj_lose_streak']++; // Increase streak on loss!
    }

    echo json_encode([
        "success" => true, "status" => $state["status"], "message" => $message,
        "player_hand" => $state["player_hand"], "player_score" => $pScore,
        "dealer_hand" => $state["status"] === "playing" ? [$state["dealer_hand"][0]] : $state["dealer_hand"],
        "dealer_score" => $state["status"] === "playing" ? calculateScore([$state["dealer_hand"][0]]) : calculateScore($state["dealer_hand"]),
        "new_balance" => $_SESSION["tokens"], "winnings" => 0
    ]);
    exit;
}
...
if ($action === "stand") {
    ...
    // Evaluate Win/Loss
    if ($dScore > 21) {
        $state["status"] = "win"; $message = "DEALER BUSTS!"; $winnings = $state["bet"] * 2;
        $_SESSION['bj_lose_streak'] = 0;
    } elseif ($pScore > $dScore) {
        $state["status"] = "win"; $message = "YOU WIN!"; $winnings = $state["bet"] * 2;
        $_SESSION['bj_lose_streak'] = 0;
    } elseif ($pScore < $dScore) {
        $state["status"] = "lose"; $message = "DEALER WINS";
        $_SESSION['bj_lose_streak']++;
    } else {
